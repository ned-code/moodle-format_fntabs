            ...
            if (!empty($section)) {
                if (!empty($move) and confirm_sesskey()) {
                    if (!move_section($course, $section, $move)) {
                        echo $OUTPUT->notification('An erroree occurred while moving a section');
                    }
                }
            }
        } else {
 ...
 *
 */
    function print_header() { //change by sudhanshu from header to head
 ...
 */
    function print_body() {
        ...
        global $CFG, $USER, $SESSION, $COURSE, $THEME, $course,$DB;
       ...
       get_all_mods($course->id, $mods, $modnames, $modnamesplural, $modnamesused);
        ...
        if (! $sections = get_all_sections($course->id)) {   // No sections found
            ...
            #get_records('role_capabilities', array('roleid'=>$roleid, 'contextid'=>$cid)))
           # $DB->get_records('course_sections',array('course'=>$course->id, 'section'=>0));
 ...
 *
 */
    function print_sections($course, $section, $mods, $modnamesused, $absolute=false, $width="100%", $return=false) {
        if ($return) {
            ob_start();
        }
        print_sections($course, $section, $mods, $modnamesused, $absolute, $width);
        if ($return) {
